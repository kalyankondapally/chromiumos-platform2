...
void sl_vifio_initialize_gpu_context() {
  ...
  char* env_force_blit = getenv("FORCE_BLIT");
  if ((env_force_blit != NULL) && (is_virtio_gpu == 1)) {
    force_blit = 1;
  }
...
static int sl_initialize_egl_resource(struct sl_crosvm_data* meta_data, bool target) {
  ...
  size_t size = ALIGN(sl_size_for_drm_format(meta_data->format, meta_data->height, meta_data->stride[0]), pagesize);
...
void sl_vifio_create_prime_buffer(struct wl_client* client,
  ...
  host_buffer->target_buffer = static_cast<sl_crosvm_data*>(malloc(sizeof(*host_callback)));
  ...
  host_buffer->source_buffer = static_cast<sl_crosvm_data*>(malloc(sizeof(*host_callback)));
  ...
  struct drm_i915_gem_get_tiling get_tiling = { .handle = prime_handle.handle, .tiling_mode=0 };
  ...
  if (prime_handle.handle > 0) {
    struct drm_gem_close gem_close;
    memset(&gem_close, 0, sizeof(gem_close));
    gem_close.handle = prime_handle.handle;
    if (gen_ioctl(source->drm_fd, DRM_IOCTL_GEM_CLOSE, &gem_close))
  ...
  target->addr = static_cast<uint8_t*>(mmap(NULL, size, PROT_WRITE, MAP_SHARED, target->virtio_id, 0));
  if (!force_blit) {
    return;
  }
